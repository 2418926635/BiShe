<template>
    <div class="view-content-comment" style="margin-bottom: 100rpx;">
        <span class="title">{{ title }}</span>
        <div class="view-conmment-note" v-if="commentList.length == 0">
            <!-- <img src="https://ossweb-img.qq.com/images/lol/web201310/skin/big10006.jpg" class="icon-none" width="20"
                height="20"> -->
            <span class="text-conmment-note">暂无评论</span>
        </div>
        <div v-if="commentList.length > 0" class="view-comment-list" v-for="(item, index) in commentList" :key="index">
            <img :src="item.HEADIMGURL" class="view-comment-list-item-photo" width="50" height="50">
            <div class="view-comment-list-item">
                <div class="comment-title">
                    <span class="comment-name">{{ item.FIRSTNICKNAME }}</span>
                    <!-- <div style="flex: 1;"></div>
                    <img v-if="item.IS_PRAISE == 1" class="praise-icon" @click="clickPraise(item)"
                        src="https://ossweb-img.qq.com/images/lol/web201310/skin/big10006.jpg" width="20" height="20">
                    <img v-else class="praise-icon" @click="clickPraise(item)"
                        src="data:image/png;base64,"
                        width="20" height="20">
                    <span class="praise-num" style="margin-left: 10rpx;">{{ item.PRAISE_NUM }}</span> -->
                </div>
                <span class="comment-content">{{ item.COMMENT }}</span>
                <div class="comment-sub">
                    <span class="comment-sub-time">{{ item.COMMENT_TIME }}</span>
                    <div class="comment-sub-recomment" style="color: #999999;" 
                        @click="clickDelete(item)">删除</div>
                        <!-- <div class="comment-sub-recomment" style="color: #999999;" v-if="item.CANDELETE == 1"
                        @click="clickDelete(item)">删除</div> -->
                    <!-- <div class="comment-sub-recomment" @click="clickRecomment(item)">回复</div> -->
                </div>
                <div class="comment-child" v-show="item.CHILD_ANWSER_LIST.length > 0">
                    <div class="comment-child-item" v-for="(itemChild, indexChild) in item.CHILD_ANWSER_LIST"
                        :key="indexChild">
                        <span class="comment-child-item-name" @click="clickRecommentChild(itemChild)">{{
                            itemChild.FIRSTNICKNAME + '：'
                        }}</span>
                        <!-- <span class="comment-child-item-content">{{ (itemChild.SECONDNICKNAME ? '回复 ' + itemChild.SECONDNICKNAME + '：' : '') + itemChild.COMMENT }}</span> -->
                        <!-- <span class="comment-child-item-content" v-if="itemChild.SECONDNICKNAME">回复</span> -->
                        <span class="comment-child-item-content">{{
                            itemChild.SECONDNICKNAME ? itemChild.SECONDNICKNAME
                                : ''
                        }}</span>
                        <span class="comment-child-item-content" v-if="itemChild.SECONDNICKNAME">：</span>
                        <span class="comment-child-item-content">{{ itemChild.COMMENT }}</span>
                        <!-- <span v-if="itemChild.CANDELETE == 1" class="comment-child-item-delete"
                            @click="clickDeleteChild(itemChild)">删除</span> -->

                            <span class="comment-child-item-delete"
                            @click="clickDeleteChild(itemChild)">删除</span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
export default {
    name: 'MulCommentList',
    //属性
    props: {
        commentList: {
            type: Array
        },
        title: {
            type: String,
            default: '网友评论'
        }
    },
    data() {
        return {};
    },
    methods: {
        /**
         * 点赞
         * @param {Object} item
         */
        clickPraise(item) {
            this.$emit('clickPraise', item);
        },
        /**
         * 回复 评论
         * @param {Object} item
         */
        clickRecomment(item) {
            this.$emit('clickRecomment', item);
        },
        /**
         * 删除评论
         * @param {Object} item
         */
        clickDelete(item) {
            this.$emit('clickDelete', item);
        },
        /**
         * 删除评论
         * @param {Object} item
         */
        clickDeleteChild(item) {
            this.$emit('clickDeleteChild', item);
        },
        /**
         * 回复评论的评论
         * @param {Object} item
         */
        clickRecommentChild(item) {
            this.$emit('clickRecommentChild', item);
        }
    }
};
</script>

<style lang="scss">




.title{
    text-align: left;
    margin: 1%;
}

.comment-content{
    margin-bottom: 10px;
}
.comment-sub{
    margin-bottom: 20px;
}
.comment-child-item{
    margin-bottom: 10px;
}


// @import '@/common/css/iconfont.css';
.view-content-comment {
    background-color: white;
    margin-top: 20rpx;
    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 30rpx;

    .title {
        width: 100%;
        color: #333;
        font-weight: bold;
        font-size: 34rpx;
    }
}

.praise-icon {
    height: 40rpx;
    width: 40rpx;
    font-size: 36rpx;
    color: #999;
}

.praise-num {
    font-size: 26rpx;
    color: #999;
}

.view-conmment-note {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    flex: 1;
    padding-bottom: 20rpx;
}

.icon-none {
    width: 200rpx;
    height: 200rpx;
    color: #bbbbbb;
}

.text-conmment-note {
    color: #808080;
    font-size: 32rpx;
    margin-top: 30rpx;
}

.border-line {
    border-bottom: 1px solid #f6f6f6;
}

.view-comment-list {
    padding-top: 20px;
    width: 100%;
    display: flex;
    flex-direction: row;
    background-color: white;
    padding: 20rpx 0;
    border-bottom: 1px solid #f6f6f6;
}

.view-comment-list-item-photo {
    height: 80rpx;
    width: 80rpx;
    border-radius: 40rpx;
    border: 1px solid #f6f6f6;
}

.view-comment-list-item {
    margin-bottom: 20px;
    margin-left: 10px;
    width: 100%;
    margin-left: 16rpx;
    display: flex;
    flex-direction: column;
    align-items: flex-start;
}

.comment-name {
    color: #666;
    font-size: 32rpx;
    display: flex;
    align-items: center;
}

.comment-title {
    align-items: center;
    display: flex;
    height: 60rpx;
    flex-direction: row;
}

.comment-content {
    color: #000000;
    font-size: 32rpx;
    font-weight: 500;
    width: 100%;
    text-align: left;
}

.comment-sub {
    width: 100%;
    display: flex;
    align-items: center;
    flex-direction: row;

    .comment-sub-time {
        color: #999;
        font-size: 26rpx;
    }

    .comment-sub-recomment {
        margin-left: 10px;
        color: #666;
        font-size: 28rpx;
        padding: 0 0 0 20rpx;
    }
}

.comment-child {
    padding: 10px;
    width: 100%;
    background-color: #f8f8f8;
    border-radius: 10rpx;
    padding: 0 20rpx;
}

.comment-child-item {
    flex-wrap: wrap;
    margin: 15rpx 0;
    display: flex;
    align-items: center;
    flex-direction: row;

    .comment-child-item-name {
        white-space: nowrap;
        font-size: 30rpx;
        color: #5074cf;
    }

    .comment-child-item-content {
        font-size: 30rpx;
        text-align: center;
    }

    .comment-child-item-delete {
        font-size: 28rpx;
        color: #999999;
        margin-left: 30rpx;
        text-align: center;
    }
}
</style>
