    ...
    <div class="user-main" v-loading="pendingflag">
                    ...
                    <el-row :gutter="20">
                        <el-col :span="8">
                            <div class="demo-input-suffix">
                                标题：
                                <el-input placeholder="请输入内容" v-model="search.result3" class="search-user">
                                </el-input>
                            </div>
                        </el-col>
                        <el-col :span="4">
        ...
        <el-dialog :title="roleinfotitle" :visible.sync="dialogroleVisible" width="600px" append-to-body
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="反馈" prop="title">
                            <el-input v-model="form.result10" placeholder=" " />
                        </el-form-item>
                    </el-col>
                    <!-- <el-col :span="24">
...
<script>
import { updatefkbasemsg,querybasemsg, insertbasemsg, updatebasemsg, updatebasemsgstatus, basemsgremove } from '@/api/sysapi'
...
export default {
    ...
    data() {
        return {
            ...
            griddata: [
                { prop: 'datetime', label: '报修时间', width: '' },
                // { prop: 'result1', label: 'result1', width: '200' },
                { prop: 'result2', label: '报修类型', width: '' },
                { prop: 'result3', label: '标题', width: '' },
                { prop: 'result4', label: '详情', width: '' },
             
                // { prop: 'result6', label: 'result6', width: '200' },
                // { prop: 'result7', label: 'result7', width: '200' },
                // { prop: 'result8', label: 'result8', width: '200' },
                // { prop: 'result9', label: 'result9', width: '200' },
                // { prop: 'result10', label: 'result10', width: '200' },
                { prop: 'userName', label: '报修人', width: '' },
                { prop: 'result10', label: '反馈情况', width: '' },
            ],
            tableData: [],
            fileListwl: [],
            multipleSelection: [],
    ...
    methods: {
        ...
        query() {
            ...
            let obj = {
                ...
                result3: this.search.result3,
                size: this.size,
                status: this.search.status == "正常" ? 0 : 1
            }
            // this.pendingflag = true
            querybasemsg(obj).then(res => {
        ...
        userinfosubmit() {
            ...
            if (this.roleinfotitle == "新增") {
                ...
                updatefkbasemsg(objedit).then(res => {
                    this.query()
                    this.$message.success('操作成功')
                    this.dialogitemVisible = false
                }).catch(error => {
