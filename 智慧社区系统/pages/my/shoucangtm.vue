				...
				<view class="content">
					...
					<view class="desc" style="line-height: 1.5;">
						...
						<view class="grid col-6">
							...
							<view @click="delpingjia(val)" class="text-right" style="color: red;font-weight:bold;">取消收藏</view>
	...
	export default {
		...
		methods: {
			async init(){
				...
				let resdata = await new this.Request(this.Urls.m().querycollectionuser, obj).modepost()
				that.list = resdata
			...
			async delpingjia(val){
				...
				let resdata = await new this.Request(this.Urls.m().removecollectionid, obj).modepost()
				if(resdata == 'ok'){
					uni.showToast({
						title: '操作成功',
						duration: 2000
