			...
			<view class="card-main" v-for="(item,i) in listdata" :key="i" style="margin-bottom: 20px;">
				...
				<view class="card-top">
					...
					<view class="card-toptitle02">
						<!-- <text style="color: limegreen;font-weight: bold;margin-right: 10px;" @click="seeitem(item.id,item)">查看</text> -->
						<text style="color: limegreen;font-weight: bold;margin-right: 10px;" @click="updatafun(item.id,item)">修改</text>
	...
	export default {
		...
		methods: {
			...
			async query() {
				...
				let resdata = await new this.Request(this.Urls.m().queryannouncementbyuser, obj).modepost()
				that.listdata = resdata
			},
			updatafun(val, item) {
				uni.navigateTo({
					url: `./usergoodsmydetail?type=updata&id=${val}`
				});
			},
			addproject() {
				uni.navigateTo({
			...
			async deletefunfun(val) {
				...
				let resdata = await new this.Request(this.Urls.m().removeannouncement, obj).modepost()
				that.query()
			}
