		...
		<view class="formcontent">
			<form>
				<view class="cu-form-group">
					...
					<input disabled placeholder="户号" v-model="form.result3" name="input"></input>
				</view>
				<view class="cu-form-group">
					<view class="title">户主姓名</view>
					<input disabled placeholder="户主姓名" v-model="form.result4" name="input"></input>
				</view>
				<view class="cu-form-group">
					<view class="title">缴费金额</view>
					<input type="number" placeholder="缴费金额(元)" v-model="form.result5" name="input"></input>
				</view>
			</form>
	...
	export default {
		...
		methods: {
			...
			async submit() {
				...
				let resdata = await new this.Request(this.Urls.m().insertbasemsg, obj).modepost()
				if (resdata == 'success') {
					uni.showToast({
						title: '缴费成功',
						icon: 'success',
